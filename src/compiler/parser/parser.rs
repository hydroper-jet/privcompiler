...
impl<'input> Parser<'input> {
    ...
    fn pop_location(&mut self) -> Location {
        self.locations.pop().unwrap().combine_with_start_of(self.previous_token.1.clone())
    }
    ...
    */
    ...
    pub(crate) fn refine_metadata(&self, exp: &Rc<Expression>) -> Result<Rc<UnprocessedMetadata>, MetadataRefineError> {
    ...
    fn refine_metadata_entries(&self, list: &Vec<Rc<Expression>>) -> Result<Vec<Rc<UnprocessedMetadataEntry>>, MetadataRefineError> {
        let mut r = Vec::<Rc<UnprocessedMetadataEntry>>::new();
        for entry in list {
    ...
    fn refine_metadata_entry(&self, exp: &Rc<Expression>) -> Result<Rc<UnprocessedMetadataEntry>, MetadataRefineError> {
        match exp.as_ref() {
            Expression::Assignment(AssignmentExpression { compound, left, right, location }) => {
                ...
                Ok(Rc::new(UnprocessedMetadataEntry {
                    location: location.clone(),
                    key: Some(key),
                    value: Rc::new(value),
                }))
        ...
        match exp.as_ref() {
            ...
            Expression::Call(CallExpression { base, arguments, .. }) => {
                ...
                match name.unwrap().0.as_ref() {
                    "File" => self.refine_metadata_file_value(arguments, &exp.location()),
                    "List" => {
                        let entries = self.refine_metadata_entries(arguments)?;
                        Ok(UnprocessedMetadataValue::List((entries, exp.location())))
                    },
    ...
    fn refine_metadata_file_value(&self, list: &Vec<Rc<Expression>>, location: &Location) -> Result<UnprocessedMetadataValue, MetadataRefineError> {
        if list.len() != 1 {
            return Err(MetadataRefineError::Syntax);
        }
        ...
        match list[0].as_ref() {
            Expression::StringLiteral(StringLiteral { value, location }) => {
                path = (value.clone(), location.clone());
            },
            Expression::Binary(BinaryExpression { left, operator, right, .. }) => {
                ...
                path = (right_val.clone(), right.location());
            },
            _ => {
                return Err(MetadataRefineError::Syntax);
            },
