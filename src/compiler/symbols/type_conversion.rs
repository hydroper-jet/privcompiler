...
pub enum TypeConversionRelationship {
    ...
    FromAnyPromiseToSubtypePromise,
}

pub struct TypeConversions<'a>(pub &'a SymbolHost);

impl<'a> TypeConversions<'a> {
    pub fn implicit_constant_conversion(&self, value: &Symbol, target_type: &Symbol) -> Result<Option<Symbol>, DeferVerificationError> {
        ...
        if value.is_undefined_constant() {
            if target_type.is_nullable_type() {
                ...
                return Ok(Some(self.0.factory().create_enum_constant(v, target_type)));
        ...
        if value.is_number_constant() {
            ...
            self.0.long_type().throw_if_unresolved()?;
            self.0.single_type().throw_if_unresolved()?;

            let number_type = self.0.number_type();
            number_type.throw_if_unresolved()?;
            ...
            big_int_type.throw_if_unresolved()?;
    ...
    pub fn implicit_conversion(&self, value: &Symbol, target_type: &Symbol, optional: bool) -> Result<Option<Symbol>, DeferVerificationError> {
        let from_type = value.static_type(self.0);
        if &from_type == target_type {
            return Ok(Some(value.clone()));
        }
        ...
        if const_conv.is_some() {
            return Ok(const_conv);
        }

        // From *
        if from_type.is_any_type() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromAny, optional, target_type)));
        }

        // To *
        if target_type.is_any_type() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::ToAny, optional, target_type)));
        }
        ...
        if self.0.is_numeric_type(&from_type_non_null) && self.0.is_numeric_type(&target_type_non_null) && !(from_nullable && to_non_nullable) {
            ...
            if compatible {
                return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::BetweenNumberTypes, optional, target_type)));
            }
        }
        ...
        if from_non_nullable && target_type_non_null.is_ascending_type_of(&from_type, self.0) {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNonNullableToCovariantType, optional, target_type)));
        }
        ...
        if from_nullable && to_nullable && target_type.base().is_ascending_type_of(&from_type.base(), self.0) {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNullableToNullableCovariantType, optional, target_type)));
        }

        // From T to T?
        if from_non_nullable && to_nullable && from_type == target_type_non_null {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNonNullableToNullable, optional, target_type)));
        }
        ...
        if from_interface && target_type_non_null == object_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromInterfaceToObject, optional, target_type)));
        }
        ...
        if from_nullable && from_type.base().is_interface_type() && to_nullable && target_type.base() == object_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNullableInterfaceToNullableObject, optional, target_type)));
        }

        Ok(None)
    }

    pub fn explicit_conversion(&self, value: &Symbol, target_type: &Symbol, optional: bool) -> Result<Option<Symbol>, DeferVerificationError> {
        let from_type = value.static_type(self.0);
        if &from_type == target_type {
            return Ok(Some(value.clone()));
        }
        ...
        if from_type.is_class_type() || from_type.is_enum_type() {
            for proxy in from_type.list_of_to_proxies(self.0).iter() {
                ...
                if result_type.is_equals_or_subtype_of(&target_type, self.0) {
                    return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::ThroughToProxy, optional, target_type)));
                }
            }
        }
        ...
        if from_non_nullable && target_type.is_subtype_of(&from_type, self.0) {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNonNullableToContravariantType, optional, target_type)));
        }

        // From nullable to non-nullable contravariant type
        if from_nullable && target_type.is_subtype_of(&from_type, self.0) {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNullableToNonNullableContravariantType, optional, target_type)));
        }
        ...
        && target_type_non_null.type_after_substitution_has_origin(&array_type) {
            ...
            if to_el_type.is_subtype_of(&from_el_type, self.0) {
                return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::ArrayToContravariantArray, optional, target_type)));
            }

            // Array to covariant Array
            if to_el_type.is_ascending_type_of(&from_el_type, self.0) {
                return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::ArrayToCovariantArray, optional, target_type)));
            }
        }
        ...
        string_type.throw_if_unresolved()?;

        // From String to enum
        if from_type == string_type && target_type_non_null.is_enum_type() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromStringToEnum, optional, target_type)));
        }

        // From number to enum
        if target_type_non_null.is_enum_type() && from_type == target_type_non_null.enumeration_numeric_type().unwrap() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNumberToEnum, optional, target_type)));
        }
        ...
        if from_type == string_type && target_type_non_null == char_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromStringToChar, optional, target_type)));
        }

        // From Char to String
        if from_type == char_type && target_type_non_null == string_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromCharToString, optional, target_type)));
        }
        ...
        if from_type == char_type && target_type_non_null == number_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromCharToNumber, optional, target_type)));
        }

        // From Number to Char
        if from_type == number_type && target_type_non_null == char_type {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromNumberToChar, optional, target_type)));
        }
        ...
        if from_type.is_map_type_of_any_any(self.0) && target_type_non_null.is_class_type() && target_type_non_null.allow_literal() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromMapToLiteralClass, optional, target_type)));
        }
        ...
        if from_type.is_class_type() && from_type.allow_literal() && target_type_non_null.is_map_type_of_any_any(self.0) {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromLiteralClassToMap, optional, target_type)));
        }

        // From type parameter T to W
        if from_type.is_type_parameter_type() {
            return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromTypeParameter, optional, target_type)));
        }

        let from_promise_result_type = from_type.promise_result_type(&self.0)?;
        if let Some(from_promise_result_type) = from_promise_result_type {
            let to_promise_result_type = target_type.promise_result_type(&self.0)?;
            if let Some(to_promise_result_type) = to_promise_result_type {
                if from_promise_result_type == self.0.any_type() && to_promise_result_type.is_subtype_of(&from_promise_result_type, &self.0) {
                    return Ok(Some(self.0.factory().create_conversion_value(value, TypeConversionRelationship::FromAnyPromiseToSubtypePromise, optional, target_type)));
                }
            }
        }
